...
We test ngscopeclient on the following platforms:

\begin{itemize}
\item Linux: Arch, Debian 12 / 13, Fedora 43, Ubuntu 22.04 / 24.04
\item MacOS: Sequoia
\item Windows: 11
\end{itemize}
...
We do not test on Windows 10, however there are no known incompatibilities.

\subsection{Processors}

Any 64-bit Intel, AMD, or ARM processor (including Apple Silicon) should be able to run ngscopeclient.

If AVX2 and/or AVX512F support is present ngscopeclient will use special optimized versions of some signal processing
functions, however neither instruction set is required.

We don't actively test on 32-bit platforms due to the significant RAM requirement of typical workloads, but we won't stop you from trying and would love to hear if you get it working.
...
On Linux, if you do not have a supported GPU you can still use ngscopeclient with software rendering via llvmpipe, however performance is likely to be poor.
...
The maximum supported waveform size depends on your Vulkan implementation but is typically $2^{32}$ bytes (4 GB) assuming
sufficient video memory is available. This translates to one gigapoint analog or four gigapoints digital.
...
packages for the officially supported platforms are automatically built each commit by our CI pipeline. Unlike release
packages, CI builds are not signed so you may encounter security warnings on some platforms when installing them.

Development binaries are available on our CI artifact server at \url{https://dl1.ngscopeclient.org/ngscopeclient-ci/}. Artifacts are organized in folders by year and month, then by date/time and commit hash of the specific build. Old builds are periodically deleted to recover disk space; the artifact server usually has a month or so worth of historical binaries depending on the pace of development.
...
sudo apt-get install build-essential git cmake pkgconf libgtk-3-dev libsigc++-2.0-dev libyaml-cpp-dev catch2 libglfw3-dev curl xz-utils libhidapi-dev lsb-release dpkg-dev file
\end{lstlisting}
...
Basic requirements:
\begin{lstlisting}[language=sh, numbers=none]
sudo apt install build-essential git cmake pkgconf libgtk-3-dev libsigc++-2.0-dev libyaml-cpp-dev catch2 libglfw3-dev curl xz-utils libhidapi-dev lsb-release
\end{lstlisting}
...
To build the documentation, you will also need LaTeX packages:
\begin{lstlisting}[language=sh, numbers=none]
sudo dnf install texlive texlive-tex4ht texlive-dvipng texlive-make4ht texlive-luaxml texlive-makecell texlive-tocloft texlive-inconsolata texlive-gensymb texlive-newtx texlive-upquote
\end{lstlisting}
...
\begin{figure}[h]
\centering
	\bigimage{ng-images/empty-window.png}
