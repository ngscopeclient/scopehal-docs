...
Calculates the time interval error of a test data or clock signal with respect to an ideal ``golden" clock (typically
obtained from the \hyperref[filter:cdrpll]{Clock Recovery (PLL)} block). The TIE can be either plotted directly to look
for time-varying patterns in jitter (e.g. from transmit clock instability) or used as input to other filters to perform
more complex jitter analysis.

Each time the test signal crosses the threshold level (if analog) or toggles (if digital) the corresponding edge in the
golden clock is located and the difference in time between the two is reported as the time interval error for that
sample.

\textbf{Notes:}
\begin{itemize}
\item If the PLL does not start in a fully locked state (e.g. if the test signal is slightly off the PLL's nominal
frequency due to spread spectrum modulation) the TIE filter may report very large jitter values due to beating between
the golden and recovered clocks, which can throw off jitter analysis. The ``Skip Start" parameter allows truncating the
beginning of the dataset to avoid this issue.
\item By convention, recovered clocks in ngscopeclient have 90\textdegree{} phasing so the recovered clock edges are
centered in the data eye and can be directly used to sample the data. The TIE filter applies an internal
-90\textdegree{} phase shift to compensate, which must be accounted for when using a golden clock not generated by the
\hyperref[filter:cdrpll]{Clock Recovery (PLL)} block.
\end{itemize}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/tie.png}
\caption{Example of measuring TIE and jitter histogram of a 10.3125 Gbps Ethernet signal}
\label{filter_tie}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-tie.png}
\caption{Example filter graph showing usage of TIE filter to display a jitter histogram}
\label{filter_graph_tie}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Clock & Analog or digital & Test signal\\
\thinhline
Golden & Sparse digital & Golden PLL reference output\\
\thickhline
\end{tabularx}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Skip Start & Integer (s) & Ignore this much time after the start of the waveform before generating output\\
\thinhline
Threshold & Float (V) & Level to threshold analog inputs at. Ignored for digital inputs\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This filter outputs a sparse analog sample with one sample per level crossing or toggle of the test signal containing
the calculated TIE value.
...
clock. Additionally, a sampled data stream is provided containing data values at each recovered clock edge which may be
fed directly to various protocol decode blocks.
...
With analog inputs, this filter uses a single threshold suitable for NRZ inputs. Using either the upper or lower eye
midpoint as a threshold will \emph{work} on a MLT-3 signal (since the -1 to +1 transition is not allowed by the
protocol) however high frequency jitter will increase slightly since the PLL is not able to track crossings of the
lower eye in this configuration and will run open-loop a larger fraction of the time than normal.

Using a single threshold for a PAM signal is not recommended and will add data-dependent jitter (since the slew rate
for, for example, a -2 to +2 transition is different than that for a -1 to +1 transition), potentially severe enough to
completely close the eye. For proper results on PAM signals, use the \hyperref[filter:pamedge]{PAM edge detector}
filter to find level crossings and then apply the CDR to the detected transitions. Using the PAM edge detector will
also give better tracking of MLT-3 although it is not strictly necessary in this case.
...
\caption{Example filter graph showing usage of CDR PLL filter with recovered clock used for eye pattern and sampled
data for a protocol decode}
\label{filter_cdrpll}
\end{figure}
...
This filter outputs an digital waveform with one clock sample per transition of the recovered clock, and a sampled data
waveform (analog or digital as appropriate) with one sample per transition of the recovered clock.
...
This filter takes a pair of I/Q streams and outputs a constellation diagram.
...
\label{filter_graph_constellation}
\end{figure}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
I & Sparse analog & Real part of input signal\\
\thinhline
Q & Sparse analog & Imaginary part of input signal\\
\thickhline
\end{tabularx}
...
\thinhline
Modulation & Enum & Constellation shape to use for EVM calculations\\
\thinhline
...
\item \textbf{v0.2:} This filter no longer needs the clock input that was required in v0.1.
Instead, threshold the sampledData output of the \hyperref[filter:cdrpll]{Clock Recovery (PLL)} and pass this to the
constellation block, as shown in Fig. \ref{filter_graph_constellation}.
\end{itemize}
...
Given waveforms from both coupled ports of a dual directional coupler and the 4-port S-parameters of the coupler,
de-embeds the coupler response in order to recover the forward and reverse waveforms.
...
Both coupled-port waveforms must be the same sample rate, memory depth, and phase aligned with respect to each other.
Use of phase-matched cables or manual deskew in postprocessing is very important especially for higher harmonics: since
the de-embed algorithm relies on subtracting a modified version of one waveform from the the other, any time shift will
increase leakage between the ports and degrade the quality of the de-embed. If the phase error is large enough the
de-embedded waveforms can even have \emph{more} leakage between paths than in the original signal, although with a
reversed sign.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-coupler-dbed.png}
\caption{Example filter graph showing de-embed of differential signals traveling through Mini-Circuits ADCB-20-82+
couplers}
\label{filter_graph_coupler_dbed}
\end{figure}
...
This filter outputs uniform analog waveforms containing waveforms de-embedded to the coupler's forward and reverse
input ports.
...
Saves waveform data to a comma-separated-value file which can be imported into a spreadsheet, external script, or
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-csv-export.png}
\caption{Example filter graph showing usage of CSV export block}
\label{filter_graph_csv_export}
\end{figure}
...
The CSV file will consist of a header line with name ``Time (s)" for the first column, and the names of the filter
graph inputs for subsequent columns. There is no escaping of commas in the filter block names, so column names may be
corrupted if a comma is present in a block name.
...
Lines starting with a \# are assumed to be comments, and not parsed as sample data. If Digilent WaveForms format header
comments are detected at the start of the file, they are parsed to extract timestamps and other metadata. If
non-numeric characters are found in the first non-comment row of the file, the fields are interpreted as channel names
and propagated to the output streams of the filter.

\begin{figure}[h]
\centering
\medimage{ng-images/filters/graph-csv-import.png}
\caption{Example filter graph showing usage of CSV import block}
\label{filter_graph_csv_import}
\end{figure}
...
\thinhline
X Axis Unit & Enum & Unit used for row timestamps\\
\thinhline
Y Axis Unit N & Enum & Unit used for Y axis values in column N\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This filter outputs one stream for each column in the CSV, other than the first which is assumed to be a timestamp.
Timestamps must be monotonic (each sample's timestamp must be greater than the one before) but need not be evenly
spaced.

If all values in the first 10 non-header lines for a given column are ``0" or ``1", with no decimal point, the column
is interpreted as digital and parsed as such. Otherwise, it is assumed to be analog. There is currently no way to
override this detection heuristic, but it may be added in the future.

After the import is complete, some statistics are computed to the intervals between timestamps to determine if the data
is uniformly sampled. If the standard deviation of the intervals is greater than 2\% of the average interval, or if the
maximum sample interval is more than 1.5 times the minimum interval, the data is assumed to be sampled irregularly and
a sparse waveform is created. If not, the data is assumed to be uniformly sampled (with any variability in sample
timestamps due to rounding error) and a uniform waveform is greated. There is currently no way to override this
detection heuristic, but it may be added in the future.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{CTLE}

Calculates S-parameters of a continuous-time linear equalizer with the specified pole/zero combination. As of now only
two poles and one zero are supported, but higher order CTLE support may be implemented in the future.

Typically, this filter will be chained with a \hyperref[filter:channelemu]{Channel Emulation} block to actually apply
the equalizer to a time-domain signal.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/ctle.png}
\caption{Example usage of CTLE block, comparing eyes of a signal before and after equalization}
\label{filter_ctle}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-ctle.png}
\caption{Example filter graph showing usage of CTLE block, comparing eyes of a signal before and after equalization}
\label{filter_graph_ctle}
\end{figure}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
DC Gain & Float & Gain, in dB, of the equalizer at DC\\
\thinhline
End Frequency & Int & Upper end of the S-parameter sweep\\
\thinhline
Pole Frequency 1 & Int & Frequency of the first pole\\
\thinhline
Pole Frequency 2 & Int & Frequency of the second pole\\
\thinhline
Zero Frequency & Int & Frequency of the first zero\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This filter outputs log magnitude and phase angle values which can be passed to any other filter block that accepts
an S-parameter input.
...
\begin{itemize}
\item \textbf{v0.2:} This filter now outputs S-parameters which must be passed to the
\hyperref[filter:channelemu]{Channel Emulation}  filter to apply them to a time-domain signal. The original version
of the filter performed the channel emulation internally.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Current Shunt}
\label{filter:currentshunt}
...
Calculates the duty-cycle distortion for a serial data stream. This is the component of data-dependent jitter which is
attributable to asymmetry between rising and falling edges.

The DCD calculation involves examining the DDJ table and splitting the measurements up into two bins, one that ends in
a rising edge and one ending in a falling. All of the DDJ measurements in each bin are averaged, giving one value for
the average jitter of a rising edge and one for the jitter of a falling. The difference between the two gives the final
DCD measurement.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/dcd.png}
\caption{Example usage of DCD block}
\label{filter_dcd}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-dcd.png}
\caption{Example filter graph showing usage of DCD block}
\label{filter_graph_dcd}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
DDJ & Protocol & Output of DDJ measurement\\
\thickhline
\end{tabularx}
...
This filter outputs an analog scalar with the computed DCD measurement.
...
remove the effects of other (random and periodic) jitter, leaving only the DDJ.  The final DDJ value is reported as the
difference between the minimum and maximum histogram bins.
...
\centering
\bigimage{ng-images/filters/dcd.png}
\caption{Example usage of DDJ block}
\label{filter_ddj}
\end{figure}
...
\centering
\bigimage{ng-images/filters/graph-dcd.png}
\caption{Example filter graph showing usage of DDJ block}
\label{filter_graph_ddj}
\end{figure}
...
\thinhline
sampledThreshold & Digital & Thresholded digital sample values\\
\thickhline
\end{tabularx}
...
\item \textbf{v0.2:} This filter no longer needs the clock input that was required in v0.1.
Instead, threshold the sampledData output of the \hyperref[filter:cdrpll]{Clock Recovery (PLL)} and pass this to the
sampledThreshold block, as shown in Fig. \ref{filter_graph_ddj}.
\end{itemize}
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/ddr1.png}
\caption{Example usage of DDR1 command bus decode}
\label{filter_ddr1}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-ddr1.png}
\caption{Example filter graph showing usage of DDR1 command bus decode}
\label{filter_graph_ddj}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
CLK & Digital & DDR clock\\
\thinhline
WE\# & Digital & DDR write enable\\
\thinhline
RAS\# & Digital & DDR row address strobe\\
\thinhline
CAS\# & Digital & DDR column address strobe\\
\thinhline
CS\# & Digital & DDR chip select\\
\thinhline
A10 & Digital & DDR address bit 10 (used as command input)\\
\thickhline
\end{tabularx}
...
This filter outputs a stream of decoded DDR1 command events.
...
\begin{figure}[h]
\centering
\medimage{ng-images/filters/graph-deskew.png}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Digital Constant}

Outputs a constant digital value of configurable width.

This filter is typically used either as a configuration strap to filters that take a digital scalar input (such as
enabling or disabling some mode), or connected to a digital GPIO channel (either a physical pin on an I/O board, or an
internal net connected to a debug IP block on an FPGA).

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-digital-constant.png}
\caption{Example filter graph showing usage of digital to constant filter}
\label{filter_digital_constant_graph}
\end{figure}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Value & Hex integer & The value to drive on the output\\
\thinhline
Width & Integer & Width of the output signal, in bits\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This filter outputs a digital scalar containing the requested value.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Digital to Analog}

Converts a digital value to an analog value using configurable units, gain, and offset. This is typically used to
visualize data captured from FPGA debug IPs as digital samples.

This filter will be extended to support both waveform and scalar input in the future, but currently only supports
scalars.

The output value is computed as (input - offset) * gain.

The input may be interpreted as normalized (0x0 maps to an output of 0.0 before scaling, while a full-scale input
regardless of bit width maps to an output of 1.0 before scaling) or non-normalized (an input of 0x20 maps to 32.0
before scaling).

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-digital-constant.png}
\caption{Example filter graph showing usage of digital to analog filter}
\label{filter_digital_analog_graph}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
din & Digital scalar & Value to convert\\
\thickhline
\end{tabularx}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Gain & Float & Gain to multiply the input by\\
\thinhline
Mode & Enum & How to interpret the input data\\
\thinhline
Offset & Float & Constant offset to subtract from the input\\
\thinhline
Unit & Enum & Units to apply to the output data\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This filter outputs an analog scalar containing the scaled output value.
...
This filter supports decoding both analog captures of the raw differential signal (via a differential probe or two
single-ended inputs subtracted), and digital captures after thresholding.

Analog inputs are assumed to be differential and centered at zero, so if you are probing the output of a differential
receiver with an analog probe the signal must be thresholded before passing to this block. This configuration is shown
in Fig. \ref{filter_dpaux} and Fig. \ref{filter_dpaux_graph}.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/dpaux.png}
\caption{Example of decoding DisplayPort aux channel}
\label{filter_dpaux}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-dpaux.png}
\caption{Example filter graph showing usage of DisplayPort aux channel decode}
\label{filter_dpaux_graph}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
aux & Analog or digital & Auxiliary channel data\\
\thickhline
\end{tabularx}
...
This filter outputs a time series of DisplayPort protocol events containing accesses to both native registers and
tunneled I2C traffic.

A second output stream provides access to the tunneled I2C data which can, for example, be passed to the
\hyperref[filter:i2ceeprom]{I2C EEPROM} protocol decode.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{DisplayPort - Main Link}
\label{filter:dpmain}

Decodes the Main Link of DisplayPort

This filter is a work in progress and does not support all features of the protocol yet. Notably missing features:

\begin{itemize}
\item 2 and 4 lane configurations
\item MST mode
\item 128b/132b line coding and UHBR bit rates
\item FEC
\item Display Stream Compression
\item Color depths other than RGB24
\item Decoding blanking interval data (currently only video scanlines are decoded)
\end{itemize}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/dpmain.png}
\caption{Example of decoding DisplayPort main channel}
\label{filter_dpmain}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-dpmain.png}
\caption{Example filter graph showing usage of DisplayPort main channel decode}
\label{filter_dpmain_graph}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
data & 8B/10B & PHY layer symbols to decode\\
\thickhline
\end{tabularx}
...
This filter outputs a time series of DisplayPort protocol events containing pixel data.

As with all other video protocol decodes in ngscopeclient, scanlines are decoded to packets as both image data and hex
byte values. Pixel data on the timeline is decoded as HTML-style \#RRGGBB color codes and drawn in the appropriate
color.
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/divide.png}
\caption{Example usage of dividing a waveform by a constant}
\label{filter_add}
\end{figure}

\begin{figure}[h]
\centering
\medimage{ng-images/filters/graph-divide.png}
\caption{Example filter graph dividing a waveform by a constant}
\label{filter_graph_add}
\end{figure}
\FloatBarrier
\subsection{Inputs}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
a & Analog waveform or scalar & Dividend\\
\thinhline
b & Analog waveform or scalar & Divisor\\
\thickhline
\end{tabularx}
...
If both inputs are vectors, this filter outputs a waveform containing the pairwise quotient; i.e. sample $i$ of the output
is $a[i] / b[i]$. No resampling is performed on the inputs so incorrect or unexpected results may occur if they do not
share the same timebase. Both inputs must be the same type (both sparse or both uniform), mixing sparse and uniform
(even if the sample timestamps are the same) is not allowed.

If both inputs are scalars, this filter outputs $a / b$.
...
\pagebreak
\section{Downconvert}
\label{filter:downconvert}

Performs digital downconversion by mixing a directly sampled RF signal with two phases of a local oscillator, then
outputs the quadrature mix products. No LO rejection filtering or decimation is performed in the current version of the
filter so these must be performed using external blocks.\footnote{In the future, as we build out a more complete RF
toolbox in ngscopeclient, we may merge some of these functions into the downconvert block to improve performance and
ease setup. Expect the interface to this block to change in future versions.}

The optional LOFrequency scalar input can be used to dynamically control the LO from another filter block and, if
connected, overrides the LO Frequency parameter.\footnote{This is also likely not to be the final interface of the
block; we are exploring various ways to unify parameters and scalar inputs and intend to merge them in some way in
the future.}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/downconvert.png}
\caption{Example usage of downconverting a 2.437 GHz Wi-Fi signal to zero IF and displaying a spectrogram}
\label{filter_downconvert}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-downconvert.png}
\caption{Example filter graph downconverting a 2.437 GHz Wi-Fi signal to zero IF, decimating and filtering, and
displaying a spectrogram}
\label{filter_graph_downconvert}
\end{figure}
\FloatBarrier
\subsection{Inputs}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
RF & Analog waveform & Directly sampled RF waveform\\
\thinhline
LOFrequency & Analog scalar & Optional LO frequency override\\
\thickhline
\end{tabularx}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
LO Frequency & Int & Local oscillator frequency\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This filter outputs two analog waveforms containing the I/Q mix products.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Downsample}
\label{filter:downsample}
...
\label{filter_graph_emphasisremoval}
\end{figure}
...
a 1.5 bit resolution improvement reduces the bandwith to $F_{nyquist} / 8$, or 1/16 the sampling frequency. The filter
properties dialog displays the calculated -3 dB bandwidth based on the current input sample rate.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/eres.png}
\caption{Example showing a squarewave with 1 and 2 bits of enhanced resolution}
\label{filter_eres}
\end{figure}

\begin{figure}[h]
\centering
\medimage{ng-images/filters/graph-eres.png}
\caption{Example filter graph showing usage of enhanced resolution filter}
\label{filter_graph_eres}
\end{figure}
...
\thinhline
Cutoff Frequency & Int & Calculated -3 dB cutoff of the filter\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

The output is an analog signal containing the filtered version of the input.
...
Finds the minimum and maximum of each sample in the input over time, and outputs them as separate streams. In other
words, sample N of the output is the min/max of the Nth sample of all waveforms processed by the filter.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/envelope.png}
\caption{Example showing envelope of a signal}
\label{filter_envelope}
\end{figure}

\begin{figure}[h]
\centering
\medimage{ng-images/filters/graph-envelope.png}
\caption{Example filter graph showing usage of envelope filter}
\label{filter_graph_envelope}
\end{figure}
...
The output is two analog signals called ``min" and ``max", containing the lower and upper envelope boundaries of the
signal.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Ethernet - 10baseT}
\label{filter:10baset}

Decodes the 10base-T Ethernet PCS/PMA as specified in IEEE 802.3-2018 clause 14.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/10baset.png}
\caption{Example showing decode of a 10baseT signal}
\label{filter_10baset}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-10baset.png}
\caption{Example filter graph showing usage of 10baseT decode}
\label{filter_graph_10baset}
\end{figure}
...
This filter outputs a stream of Ethernet sample objects which may be passed to upper layer decodes or exported to a
PCAPNG file.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Ethernet - 100baseT1}
\label{filter:100baset1}

Decodes the 100base-T1 single-pair / automotive Ethernet PMA/PCS, as specified in IEEE 802.3-2018 clause 96.

Due to the bidirectional nature of 100baseT1 signaling, it is \emph{not} possible to decode this protocol by simply
placing probes on the differential pair. A dedicated test fixture with two dual directional couplers must be used, and
there is typically enough crosstalk between the forward and reverse coupled path that the
\hyperref[filter:couplerdbed]{Coupler De-Embed} filter (which requires connections to both the forward and reverse
coupled ports of both differential legs) must be used to improve directivity.

Additionally, the 100baseT1 PHY uses 2D-PAM3 signaling which must be demuxed into separate I and Q channels by the
\hyperref[filter:iqdemux]{IQ Demux} block before decoding.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/100baset1.png}
\caption{Example showing decode of a 100baseT1 signal}
\label{filter_100baset1}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-100baset1.png}
\caption{Example filter graph showing usage of 100baseT1 decode}
\label{filter_graph_100baset1}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
i & Analog & Input signal, I channel\\
\thinhline
q & Analog & Input signal, Q channel\\
\thickhline
\end{tabularx}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Scrambler Polynomial & Enum & Select the correct LFSR polynomial depending on which side of the link is being decoded\\
\thinhline
Threshold I+ & Float & Decision threshold between 0 and +1 states on the I channel\\
\thinhline
Threshold I- & Float & Decision threshold between -1 and 0 states on the I channel\\
\thinhline
Threshold Q+ & Float & Decision threshold between 0 and +1 states on the Q channel\\
\thinhline
Threshold Q- & Float & Decision threshold between -1 and 0 states on the Q channel\\
\thickhline
\end{tabularx}
...
\begin{itemize}
\item \textbf{v0.2:} This filter no longer needs the clock input that was required in v0.1.
Instead, simply connect to the I/Q demux output
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Ethernet - 100baseT1 Link Training}
\label{filter:100baset1-training}

Decodes the link training stage of 100base-T1 single-pair / automotive Ethernet, as specified in IEEE
802.3-2018 clause 96. As with the \hyperref[filter:100baset1]{100baseT1} decode, this filter requires special fixturing
to acquire the necessary signals for a decode.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/100baset1-training.png}
\caption{Example showing decode of a 100baseT1 training signal}
\label{filter_100baset1_training}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-100baset1-training.png}
\caption{Example filter graph showing usage of 100baseT1 training decode}
\label{filter_graph_100baset1_training}
\end{figure}
...
This filter outputs a stream of decode state values indicating whether the link is in SEND\_Z, SEND\_I, or SEND\_N
state.
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/100basetx.png}
\caption{Example showing decode of a 100baseTX signal}
\label{filter_100basetx}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-100basetx.png}
\caption{Example filter graph showing usage of 100baseTX decode}
\label{filter_graph_100basetx}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
sampledData & Analog & Sampled data output from CDR PLL\\
\thickhline
\end{tabularx}
...
\begin{itemize}
\item \textbf{v0.2:} This filter no longer needs the clock input that was required in v0.1.
Instead, simply pass the sampledData output of the \hyperref[filter:cdrpll]{Clock Recovery (PLL)} directly to the
100baseTX decode, as shown in Fig. \ref{filter_graph_100basetx}.
\end{itemize}
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/1000basex.png}
\caption{Example showing decode of a 1000baseX signal}
\label{filter_1000basex}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-1000basex.png}
\caption{Example filter graph showing usage of 1000baseX decode}
\label{filter_graph_1000basex}
\end{figure}
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/10gbaser.png}
\caption{Example showing decode of a 10Gbase-R signal}
\label{filter_10gbaser}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-10gbaser.png}
\caption{Example filter graph showing usage of 10Gbase-R decode}
\label{filter_graph_10gbaser}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
data & 64b/66b & Output of \hyperref[proto:64b66b]{64b/66b protocol decode}\\
\thickhline
\end{tabularx}
...
Note that unlike most other Ethernet decodes, this block does \emph{not} decode QSGMII up to Ethernet frames; it only
demuxes the QSGMII to 4x SGMII. You can then use the \hyperref[filter:sgmii]{SGMII} decode block to decode any or all
of the individual PHY lanes.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/qsgmii.png}
\caption{Example of decoding TX and RX lanes of a QSGMII PHY}
\label{filter_qsgmii}
\end{figure}
...
Decodes the Reduced Gigabit Media Independent Interface as specified in the RGMII 2.0 specification. This filter
assumes that the clock and data are center-aligned such that the clock can be used directly to sample the data without
additional phase shifting. You may need to use instrument-side skew correction or \hyperref[filter:deskew]{Deskew}
block to adjust timing on the clock if you are using PHY-internal delays and have an edge-aligned clock.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/rgmii.png}
\caption{Example of decoding RGMII}
\label{filter_rgmii}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-rgmii.png}
\caption{Example filter graph showing usage of RGMII filter}
\label{filter_rgmii}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
data & Digital bus & 4-bit data bus\\
\thinhline
clk & Digital & RGMII clock \\
\thinhline
ctl & Digital & RGMII TX\_CTL or RX\_CTL signal \\
\thickhline
\end{tabularx}
...
This filter outputs a stream of Ethernet sample objects which may be passed to upper layer decodes or exported to a
PCAPNG file. In-band status is also decoded.
...
Decodes the Reduced Media Independent Interface as specified in the RMII Consortium RMII specification.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/rmii.png}
\caption{Example of decoding RMII}
\label{filter_rmii}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-rmii.png}
\caption{Example filter graph showing usage of RMII filter}
\label{filter_rmii}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
clk & Digital & RMII system clock\\
\thinhline
ctl & Digital & Enable / control line (CRS\_DV or TX\_EN depending on direction) \\
\thinhline
d0 & Digital & Data bit 0 \\
\thinhline
d1 & Digital & Data bit 1 \\
\thickhline
\end{tabularx}
...
Decodes Serial GMII data at 10, 100, or 1000 Mbps rates to Ethernet frames. Manual configuration of the data rate is
currently required: SGMII always runs at 1.25 Gbps at the physical layer and simply repeats 8b/10b symbols 10 or 100
times at lower data rates. Automatic detection of data rate may be added in the future by parsing autonegotiation
traffic or looking at the number of preamble bytes between the K27.7 start token and the D21.6 start-of-frame marker.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/sgmii.png}
\caption{Example of decoding SGMII}
\label{filter_sgmii}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-sgmii.png}
\caption{Example filter graph showing usage of SGMII filter}
\label{filter_sgmii}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
data & 8B/10B & Coded input symbols\\
\thickhline
\end{tabularx}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Speed & Enum & The current operating speed of the link\\
\thickhline
\end{tabularx}
...
This filter outputs a stream of 16-bit negotiation codewords, which is typically fed to the
\hyperref[filter:anegpage]{Ethernet Autonegotiation Page} filter.
...
\label{filter:basex_negotiation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Ethernet Clause 73 Autonegotiation}
\label{filter:clause73_negotiation}

Decodes the Base-KX autonegotiation signaling for Ethernet as specified in IEEE 802.3-2018 clause 73.
...
\begin{math}
Out_i =
	\left(1-\frac{1}{\displaystyle{2^\frac{1}{x}}}\right)\left(Out_{i-1}\right) +
	\left(\frac{1}{\displaystyle{2^\frac{1}{x}}}\right)\left(S\right)
\end{math}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/ema.png}
\caption{Example of using averaging to reduce noise on data from a UV/VIS/NIR spectrometer}
\label{filter_ema}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-ema.png}
\caption{Example filter graph showing usage of exponential moving average filter}
\label{filter_ema}
\end{figure}
\FloatBarrier
...
This filter outputs an analog waveform with one sample per sample of the input, containing the averaged waveform.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Eye Bit Rate}
\label{filter:eyebitrate}

Measures the bit rate of an eye pattern.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/eye-bitrate.png}
\caption{Example of measuring bit rate of an eye pattern}
\label{filter_eye_bitrate}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-eye-bitrate.png}
\caption{Example filter graph measuring bit rate of an eye pattern}
\label{filter_graph_eye_bitrate}
\end{figure}
\FloatBarrier
...
This filter outputs a scalar containing the calculated bit rate.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Eye Height}
\label{filter:eyeheight}

Measures the vertical opening of an eye pattern. The opening can be measured as either at a single point, or across a
range of values representing the receiver sampling aperture.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/eye-height.png}
\caption{Example of measuring height of an eye pattern}
\label{filter_eye_height}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-eye-height.png}
\caption{Example filter graph measuring height of an eye pattern}
\label{filter_graph_eye_height}
\end{figure}
\FloatBarrier
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Begin Time & Integer & Start position of measurement (eye center = 0)\\
\thinhline
End Time & Integer & End position of measurement (eye center = 0)\\
\thinhline
Midpoint Voltage & Integer & Vertical center of the eye\\
\thickhline
\end{tabularx}
...
\begin{tabularx}{16cm}{llX}
\thickhline
\textbf{Output name} & \textbf{Type} & \textbf{Description} \\
\thickhline
heightslice & Analog waveform & Plot of vertical opening vs horizontal position across the measurement window \\
\thinhline
minheight & Float & Smallest opening found within the measurement window\\
\thickhline
\end{tabularx}
...
Measures the peak-to-peak jitter of an eye pattern. The jitter can be measured as either at a single point, or across a
range of values representing the receiver sampling aperture.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/eye-ppj.png}
\caption{Example of measuring p-p jitter of an eye pattern}
\label{filter_eye_ppj}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-eye-ppj.png}
\caption{Example filter graph measuring p-p jitter of an eye pattern}
\label{filter_graph_eye_ppj}
\end{figure}
\FloatBarrier
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Begin Voltage & Float & Start position of measurement\\
\thinhline
End Voltage & Float & End position of measurement\\
\thickhline
\end{tabularx}
...
\textbf{Output name} & \textbf{Type} & \textbf{Description} \\
\thickhline
ppjslice & Analog waveform & Plot of jitter vs vertical position across the measurement window \\
\thinhline
ppj & Float & Largest jitter found within the measurement window\\
\thickhline
\end{tabularx}
...
\pagebreak
\section{Eye Period}
\label{filter:eyeperiod}

Measures the UI width of an eye pattern.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/eye-period.png}
\caption{Example of measuring period of an eye pattern}
\label{filter_eye_period}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-eye-period.png}
\caption{Example filter graph measuring period of an eye pattern}
\label{filter_graph_eye_period}
\end{figure}
\FloatBarrier
...
This filter outputs a scalar containing the calculated period.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
\section{Eye Width}
\label{filter:eyewidth}

Measures the horizontal opening of an eye pattern. The opening can be measured as either at a single point, or across a
range of values representing the receiver sampling aperture.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/eye-width.png}
\caption{Example of measuring width of an eye pattern}
\label{filter_eye_width}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-eye-width.png}
\caption{Example filter graph measuring width of an eye pattern}
\label{filter_graph_eye_width}
\end{figure}
\FloatBarrier
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Start Voltage & Float & Start position of measurement\\
\thinhline
End Voltage & Float & End position of measurement\\
\thickhline
\end{tabularx}
...
\textbf{Output name} & \textbf{Type} & \textbf{Description} \\
\thickhline
heightslice & Analog waveform & Plot of horizontal opening vs vertical position across the measurement window \\
\thinhline
minheight & Float & Smallest opening found within the measurement window\\
\thickhline
\end{tabularx}
...
\begin{figure}[h]
\centering
\bigimage{ng-images/filters/fall.png}
\caption{Example of measuring fall time of a waveform and performing statistics on the results}
\label{filter_fall}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-fall.png}
\caption{Example filter graph measuring fall time and performing statistics}
\label{filter_graph_fall}
\end{figure}
\FloatBarrier
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Start Fraction & Float & Level to begin the fall time measurement at, in percent\\
\thinhline
End Fraction & Float & Level to end the fall time measurement at, in percent\\
\thickhline
\end{tabularx}
...
\textbf{Output name} & \textbf{Type} & \textbf{Description} \\
\thickhline
trend & Analog waveform & Plot of fall time for each edge in the signal\\
\thinhline
avg & Analog scalar & Average fall time over the whole waveform\\
\thickhline
\end{tabularx}
...
Calculates a Fast Fourier Transform and displays the log magnitude response.

NOTE: As of this writing (v0.2) the peak detection algorithm is CPU based and very slow compared to the FFT itself,
especially at deep memory depths. For example, on a Xeon 8362 + AMD Radeon 9700, a 20M point FFT runs in about 2.5
milliseconds while the peak detection takes nearly 450 ms. We intend to optimize and GPU accelerate this in the
future. To avoid unexpected slowdowns when adding a FFT to a filter graph, the peak detector defaults to disabled; to
enable it simply set the "number of peaks" parameter to a nonzero value.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/fft.png}
\caption{Example showing FFT of a modulated signal and using cursors to measure total in-band power}
\label{filter_fft}
\end{figure}

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/graph-fft.png}
\caption{Example filter graph showing FFT of a scope channel}
\label{filter_graph_fft}
\end{figure}
\FloatBarrier
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Number of Peaks & Int & Maximum number of peaks to report\\
\thinhline
Peak Window & Int & Minimum spacing between two peaks for them to be reported separately\\
\thinhline
Window & Enum & FFT window function\\
\thickhline
\end{tabularx}
...
\pagebreak
\section{IQ Demux}
\label{filter:iqdemux}

Given a sampled data waveform containing consecutively sampled I and Q values, output separate
...
This filter decodes a \textit{Serial Peripheral Interface} transaction.
It samples a whole transaction of an arbitrary number of 8-bit wide bytes.

Any clocking on the clk-line outside of \#cs being pulled low does not result
in data being sampled.
Incase there is no CS signal to be measured it can be manually reconstructed
using a moving average of the clock signal.

\begin{figure}[h]
\centering
\bigimage{ng-images/filters/spi.png}
\caption{Example 1-wire decode}
\label{filter_1wire}
\end{figure}
...
\textbf{Signal name} & \textbf{Type} & \textbf{Description} \\
\thickhline
clk & Digital & Serial data clock \\
\thinhline
\#cs & Digital & Chip select. Needs to be asserted during transaction \\
\thinhline
data & Digital & Serial data input \\
\thickhline
\end{tabularx}
...
\textbf{Parameter name} & \textbf{Type} & \textbf{Description} \\
\thickhline
Byte Endianness & Enum &
	\textbf{MSB}: Bitstream gets packed into bytes with the most significant bit first \newline
	\textbf{LSB}: Bitstream gets packed into bytes with the most significant bit first \\
\thinhline
Clock Phase Alignment & Enum &
	\textbf{Sample Rising CLK}: Data-Channel gets sampled on a rising clock change \newline
	\textbf{Sample Falling CLK}: Data-Channel gets sampled on a falling clock change \\
\thinhline
Clock Polarity & Enum &
	\textbf{Idle Low}: The clock-signal is considered deasserted when the clock-line is low \newline
	\textbf{Idle High}: The clock-signal is considered deasserted when the clock-line is high\\
\thickhline
\end{tabularx}

\subsection{Output Signal}

This decoder outputs a single decoded tata stream consisting of SPI sample objects.

\begin{tabularx}{16cm}{llX}
\thickhline
\textbf{Stream name} & \textbf{Type} & \textbf{Description} \\
\thickhline
data & SPI bus & Sampled Bytes \\
\thickhline
\end{tabularx}
