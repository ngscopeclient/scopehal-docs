...
NOTE: The current implementation of this driver only works on Linux, using the linux-gpib library. Since linux-gpib is
licensed under GNU GPL, this driver is packaged separately and must be installed from
\url{https://github.com/ngscopeclient/scopehal-plugins-gpl}. A future version which interfaces with the Linux kernel
GPIB subsystem via the ioctl interface, without using linux-gpib, is planned for the future which will eliminate the
need for a plugin.
...
This transport is used by some Antikernel Labs oscilloscopes, as well as most of the bridge servers used for
interfacing libscopehal with USB/Thunderbolt oscilloscopes. It takes three arguments: hostname/IP and two port numbers.
