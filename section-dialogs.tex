...
Minimal Markdown syntax (headings, bullets, and bold font) is currently supported.\footnote{Images and links are
supported by the Markdown renderer library but the integration to properly use them is not yet finished; tables are not
supported but this will likely be added in the future.}
...
It can be opened from the \menustyle{Window | Log Viewer} menu.
...
\label{dlg:measurements}

The Measurements dialog displays scalar values, either produced by filter graph blocks or directly measured by e.g. a
multimeter channel.

It can be opened from the \menustyle{Window | Measurements} menu.

\begin{figure}[H]
\centering
\bigimage{ng-images/dialog-measurements.png}
\caption{Measurements dialog}
\label{fig:measurements}
\end{figure}

To remove a measurement, right click the channel name and select ``delete" from the context menu.

To add a measurement, drag the stream from the stream browser to a row in the measurements dialog. Measurements can
also be reordered by dragging and dropping.

Digital vector values can be displayed as decimal, hex, or binary. To change base, right click the channel name and
select the desired format from the context menu.
...
processing. The exact set of statistics displayed can vary depending on available Vulkan extensions, discrete vs
integrated graphics, etc. This data is primarily intended for developers comparing before/after performance of
optimizations and code changes.

It can be opened from the \menustyle{Debug | Performance Metrics} menu.
...
Displays the acquisition rate, in waveforms per second, for the entire session as well as for each instrument within
the session if multiple trigger groups are active.
...
Note that Teledyne LeCroy scopes perform DSP flatness corrections in firmware between the ADC and the data presented
via the API, meaning that even if the hardware only has an 8-bit ADC (non HD models), downloading data with 16-bit
precision will result in improved waveform quality.
