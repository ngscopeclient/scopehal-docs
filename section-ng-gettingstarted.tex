...
sudo apt install build-essential cmake pkg-config libglm-dev \
	libcairomm-1.0-dev libsigc++-2.0-dev libyaml-cpp-dev \
...
sudo dnf install gcc-g++ ccache make cmake pkg-config glm-devel \
	cairomm1.0-devel libsigc++30-devel libyaml-devel yaml-cpp-devel \
...
\begin{lstlisting}[language=sh, numbers=none]
brew install pkg-config cairomm glfw cmake yaml-cpp glew catch2 libomp
\end{lstlisting}

\item Install Vulkan SDK:

Download and install the Vulkan SDK from \href{https://sdk.lunarg.com/sdk/download/1.3.250.1/mac/vulkansdk-macos-1.3.250.1.dmg}{sdk.lunarg.com/sdk/download/1.3.250.1/mac/vulkansdk-macos-1.3.250.1.dmg}.

\item Build scopehal and scopehal-apps:

\begin{lstlisting}[language=sh, numbers=none]
export VULKAN_SDK=~/VulkanSDK/1.3.250.1/macOS
