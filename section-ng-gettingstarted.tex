...
\item Apple: all Apple Silicon devices (M1 and newer). Newer Intel devices with Vulkan 2 support may work but support is not guaranteed.
\end{itemize}
...
ngscopeclient can be compiled on Linux, macOS, and Windows. While the compilation process is similar, installing the prerequisites differs among platform and distro.
...
sudo apt install build-essential git cmake pkgconf libgtk-3-dev libcairomm-1.0-dev libsigc++-2.0-dev libyaml-cpp-dev catch2 libglfw3-dev
\end{lstlisting}
...
\begin{lstlisting}[language=sh, numbers=none]
sudo bash -c 'echo "deb http://deb.debian.org/debian bullseye-backports main" >> /etc/apt/sources.list.d/bullseye-backports.list'
sudo apt update
...
Basic requirements:
\begin{lstlisting}[language=sh, numbers=none]
sudo dnf install git gcc g++ cmake make pkgconf cairomm-devel gtk3-devel libsigc++30-devel yaml-cpp-devel catch-devel glfw-devel
\end{lstlisting}
...
To build the LXI component (needed if you have LXI- or VXI-11-based instruments):
\begin{lstlisting}[language=sh, numbers=none]
sudo dnf installliblxi-devel libtirpc-devel
\end{lstlisting}
...
\begin{lstlisting}[language=sh, numbers=none]
apk add git gcc g++ cmake make pkgconf cairomm-dev gtk+3.0-dev libsigc++-dev yaml-cpp-dev catch2-3 vulkan-loader-dev glslang-dev glslang-static glfw-dev shaderc-dev spirv-tools-dev
\end{lstlisting}
...
If you are using Ubuntu, you may install the
...
In most cases, you can install the SDK components from distro-provided repositories, which is covered above. If you choose not to, or are running Debian Bullseye, the following instructions cover installing and loading the Vulkan SDK.
...
When using the SDK, you will need to source the `setup-env.sh` file any time you want to compile or run ngscopeclient. For convenience, you can add this to your `.bash_profile` or equivalent:
\begin{lstlisting}[language=sh, numbers=none]
...
You will need Xcode (either from the App Store or the Apple deveoper site); after installing, run it once for it to install system components. This provides gcc, g++, make, and similar required packages.
...
Download and install the Vulkan SDK from \href{https://sdk.lunarg.com/sdk/download/1.3.275.0/mac/vulkansdk-macos-1.3.268.1.dmg}{sdk.lunarg.com/sdk/download/1.3.268.1/mac/vulkansdk-macos-1.3.275.0.dmg}.
...
https://wixtoolset.org/docsa/wix3/

\item Install git the toolchain, and the Vulkan dependencies:

\begin{lstlisting}[language=sh, numbers=none]
pacman -S git wget mingw-w64-x86_64-cmake mingw-w64-x86_64-toolchain mingw-w64-x86_64-vulkan-headers mingw-w64-x86_64-vulkan-loader mingw-w64-x86_64-shaderc mingw-w64-x86_64-glslang mingw-w64-x86_64-spirv-tools mingw-w64-x86_64-glfw mingw-w64-x86_64-cairo mingw-w64-x86_64-libsigc++ mingw-w64-x86_64-catch mingw-w64-x86_64-ffts mingw-w64-x86_64-yaml-cpp
\end{lstlisting}
...
mkdir -p $BUILD_DIR
cd $BUILD_DIR
/mingw64/bin/cmake -DCMAKE_BUILD_TYPE=Release -G"MinGW Makefiles" $SOURCE_DIR \
	-DCMAKE_INSTALL_PREFIX="$(pwd)/install"
/mingw64/bin/cmake --build . --config Release --target install
\end{lstlisting}
