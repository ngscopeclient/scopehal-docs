...
\thickhline
Internal Logic Analyzer IP & akl.ila & \\
\thickhline
\end{tabularx}

\subsection{akl.ila}

This driver interfaces to the Antikernel Labs \verb|APB_ILA| or any other RTLDebug compatible internal logic analyzer core, via \href{https://github.com/ngscopeclient/scopehal-akldebug-bridge}{scopehal-akldebug-bridge}

\section{Demo}

The ``demo" driver is a simulation-only driver for development and training purposes, and does not connect to real
hardware. It does not currently model realistic trigger behavior (waveforms are aligned to a fixed phase regardless of
trigger setting), however we hope to simulate realistic triggering in the future.
...
Four analog test signals are provided, as well as a series of digital signals.

The analog signals can be configured in three different modes using the ``ADC mode" selector:
\begin{itemize}
\item Ideal waveform with no added impairments
\item Ideal waveform with 10 mV of AWGN
\item Ideal waveform with 10 mV of AWGN through an S-parameter ISI model of a 300mm FR-4 microstrip
\end{itemize}

Analog test signals:
\begin{itemize}
\item Tone: 1.000 GHz tone
\item Ramp: 1.000 GHz steady tone plus a second tone which sweeps from 1.100 to 1.500 GHz
\item PRBS31: 10.3125 Gbps PRBS-31
\item 8B10B: 1.25 Gbps repeating two 8B/10B symbols (K28.5 D16.2)
\end{itemize}

Digital test signals:
\begin{itemize}
\item Unidirectional SPI bus transmitting an ASCII hello-world message
\item UART transmitting an ASCII hello-world message
\item 8-bit parallel bus plus clock counting from 00 to FF
\end{itemize}
...
also, but are untested. EDIT 2026-07-23: some user are reporting problems due to DSLabs software changes.
...
Due to SCPI API deprecations made in
\href{https://github.com/EEVengers/TS.NET/commit/bf1e8e0870fc1de1da8ce0386c7f1e48d5a49276}{bf1e8e0}, 0.2.x and
older ngscopeclient versions cannot be used with current ThunderScopes. You must use ngscopeclient version 0.3 or later.
...
driver (which is also aliased as ``keysight" and can be used under either name)
...
(\issue{scopehal-apps}{790}); it is unclear what other models and firmware versions may
...
This driver is in early stage and has significant limitations and bugs. We are exploring whether to merge it with the
original legacy \texttt{rs} driver or continue updating it, but no progress can be made without a volunteer maintainer
who has access to a scope for testing. If you are interested in taking over maintainership of this driver please reach
out!
...
\item The triggers for pattern and bus are missing. Many other triggers are partially implemented in the code but disabled due to bugs and will need refactoring to be usable.
\end{itemize}
