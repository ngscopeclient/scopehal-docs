...
A horizontal cursor describes a position (usually voltage) on the Y axis. As of this writing, there is no readout functionality; horizontal cursors are purely for visual/display purposes.

Horizontal cursors are attached to a waveform view; any number of views may have independent horizontal cursors at a given time.

\begin{figure}[H]
\centering
\bigimage{ng-images/horizontal-cursors.png}
\caption{Single and double horizontal cursors}
\label{horizontal-cursors}
\end{figure}
...
When a cursor is active, a dockable pop-up dialog appears displaying the value of each waveform (including analog,
digital, and protocol channels) in the group at the cursor location. If two cursors are active, both values as well as
the difference between them (if analog) is shown (Fig. \ref{vertical-cursor-x2})
...
window. In Fig. \ref{markers}, three of the four markers are visible while the fourth is in a different waveform.
...
\caption{Session with four markers, three on the currently displayed waveform and one on a future waveform}
\label{markers}
\end{figure}
